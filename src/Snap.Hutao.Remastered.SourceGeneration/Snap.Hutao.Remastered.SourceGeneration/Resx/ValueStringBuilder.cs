...
internal ref struct ValueStringBuilder
{
    ...
    public int Capacity { get => chars.Length; }
    ...
    {
        string s = chars[..pos].ToString();
        Dispose();
        return s;
    }

    /// <summary>Returns the underlying storage of the builder.</summary>
    public Span<char> RawChars { get => chars; }
    ...
    {
        if (chars[..pos].TryCopyTo(destination))
        {
            charsWritten = pos;
            Dispose();
            return true;
    ...
    public void Insert(int index, char value, int count)
    {
        ...
        chars.Slice(index, remaining).CopyTo(chars[(index + count)..]);
